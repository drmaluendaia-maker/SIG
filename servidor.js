...
// Versión: 3.1 (Ajuste para Despliegue en Render)
// =================================================================================
...
// --- MODIFICACIÓN CLAVE PARA RENDER DISK ---
// Si la variable de entorno RENDER_DISK_DIR existe (en Render), la usamos.
// Si no, usamos una carpeta 'data' local para el desarrollo.
...
// =================================================================================
// (El resto del archivo no necesita cambios, la lógica interna es la misma)
...
io.on('connection', (socket) => {
    ...
    const setupPatientEvents = () => {
        const events = {
            'register_patient': (newPatient) => { if (currentUser.role !== 'registro') return; newPatient.registeredBy = currentUser.user; newPatient.shift = getNurseShift(new Date(newPatient.horaLlegada)); patients.push(newPatient); sortPatients(); io.emit('new_patient_notification', { patient: newPatient, patientCount: patients.length }); },
            'update_patient_level': ({ id, newLevel }) => { if (currentUser.role !== 'registro') return; const p = patients.find(p => p.id === id); if (p) { p.nivelTriage = newLevel; p.ordenTriage = triageOrder[newLevel]; sortPatients(); } },
            'call_patient': ({ id, consultorio }) => { if (currentUser.role !== 'medico') return; const currentlyAttending = patients.find(pt => pt.doctor_user === currentUser.user && pt.status === 'atendiendo'); if (currentlyAttending) { currentlyAttending.status = currentlyAttending.previousStatus || 'en_espera'; delete currentlyAttending.consultorio; delete currentlyAttending.doctor_user; delete currentlyAttending.doctor_name; } const p = patients.find(p => p.id === id); if (p) { p.previousStatus = p.status; p.status = 'atendiendo'; p.consultorio = consultorio; p.doctor_user = currentUser.user; p.doctor_name = currentUser.fullName; currentlyCalled = { nombre: p.nombre, consultorio }; io.emit('update_call', currentlyCalled); setTimeout(() => { currentlyCalled = null; io.emit('update_call', null); }, 20000); } },
            'update_patient_status': ({ id, status }) => { if (currentUser.role !== 'medico') return; const p = patients.find(p => p.id === id); if (p) { p.status = status; if (status === 'ausente' || status === 'pre_internacion') { delete p.consultorio; } sortPatients(); } },
            'add_nurse_evolution': ({ id, note }) => { if (currentUser.role !== 'registro' && currentUser.role !== 'enfermero_guardia') return; const patient = patients.find(p => p.id === id) || attendedHistory.find(p => p.id === id); if (patient) { if (!patient.nurseEvolutions) patient.nurseEvolutions = []; patient.nurseEvolutions.push({ text: note, user: currentUser.fullName, timestamp: Date.now() }); } },
            'add_doctor_note': ({ id, note }) => { if (currentUser.role !== 'medico') return; const patient = patients.find(p => p.id === id) || attendedHistory.find(p => p.id === id); if (patient) { if (!patient.doctorNotes) patient.doctorNotes = []; patient.doctorNotes.push({ text: note, doctor: currentUser.fullName, timestamp: Date.now() }); } },
            'add_indication': ({ id, text }) => { if (currentUser.role !== 'medico') return; const patient = patients.find(p => p.id === id); if (patient) { if (!patient.indications) patient.indications = []; patient.indications.push({ id: crypto.randomUUID(), text, doctor: currentUser.fullName, status: 'pendiente', timestamp: Date.now() }); } },
            'update_indication_status': ({ patientId, indicationId, newStatus }) => { if (currentUser.role !== 'enfermero_guardia') return; const patient = patients.find(p => p.id === patientId); if (patient && patient.indications) { const indication = patient.indications.find(i => i.id === indicationId); if (indication) { indication.status = newStatus; indication.completedBy = currentUser.fullName; indication.completedAt = Date.now(); } } },
            'mark_as_attended': ({ patientId }) => { const patientIndex = patients.findIndex(p => p.id === patientId); if (patientIndex > -1) { const [patient] = patients.splice(patientIndex, 1); patient.attendedAt = Date.now(); patient.attendedBy = currentUser.fullName; patient.guardDay = getDoctorGuard(new Date(patient.attendedAt)); patient.disposition = 'Alta'; attendedHistory.push(patient); } },
            'hospitalize_patient': ({ patientId, data }) => { if (currentUser.role !== 'medico') return; const patientIndex = patients.findIndex(p => p.id === patientId); if (patientIndex > -1) { const [patient] = patients.splice(patientIndex, 1); patient.attendedAt = Date.now(); patient.attendedBy = currentUser.fullName; patient.disposition = 'Internado'; patient.hospitalizationData = data; attendedHistory.push(patient); } },
            'transfer_patient': ({ patientId, data }) => { if (currentUser.role !== 'medico') return; const patientIndex = patients.findIndex(p => p.id === patientId); if (patientIndex > -1) { const [patient] = patients.splice(patientIndex, 1); patient.attendedAt = Date.now(); patient.attendedBy = currentUser.fullName; patient.disposition = 'Trasladado'; patient.transferData = data; attendedHistory.push(patient); } },
            'continue_care': (patientId) => { if (currentUser.role !== 'medico') return; const historyIndex = attendedHistory.findIndex(p => p.id === patientId); if (historyIndex > -1) { const [patientToReactivate] = attendedHistory.splice(historyIndex, 1); patientToReactivate.status = 'pre_internacion'; delete patientToReactivate.consultorio; delete patientToReactivate.doctor_user; delete patientToReactivate.doctor_name; patients.push(patientToReactivate); sortPatients(); } },
            'add_observation_to_attended': ({ id, note }) => { const patient = attendedHistory.find(p => p.id === id); if (patient) { if (currentUser.role === 'registro' || currentUser.role === 'enfermero_guardia') { if (!patient.nurseEvolutions) patient.nurseEvolutions = []; patient.nurseEvolutions.push({ text: note, user: currentUser.fullName, timestamp: Date.now() }); } else if (currentUser.role === 'medico') { if (!patient.doctorNotes) patient.doctorNotes = []; patient.doctorNotes.push({ text: note, doctor: currentUser.fullName, timestamp: Date.now() }); } } },
        ...
        for (const eventName in events) { socket.on(eventName, (data) => { if (!isAuthenticated) return; events[eventName](data); saveData(); io.emit('update_patient_list', patients); io.emit('attended_history_update', attendedHistory.sort((a,b) => b.attendedAt - a.attendedAt)); }); }
    };
    setupPatientEvents();

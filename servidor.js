...
// Versión: 3.4 (Persistencia de Datos Blindada y Admin Reparado)
// =================================================================================
...
// --- MODIFICACIÓN CLAVE: Lógica de carga de datos blindada ---
const loadData = () => {
    try {
        // Cargar pacientes (si no existe, se creará al guardar el primero)
        ...
        // --- IMPORTANTE: El servidor ahora espera que estos archivos existan ---
        // Si no existen, arrojará un error y se detendrá, previniendo el borrado de datos.
        if (!fs.existsSync(USERS_FILE)) {
            throw new Error(`Error Crítico: El archivo 'usuarios.json' no se encuentra en la carpeta 'data'.`);
        }
        if (!fs.existsSync(PRESETS_FILE)) {
            throw new Error(`Error Crítico: El archivo 'presets.json' no se encuentra en la carpeta 'data'.`);
        }
        ...
        // Detener el servidor si los archivos de configuración no existen para evitar problemas.
        process.exit(1);
    }
};
...
io.on('connection', (socket) => {
    ...
    // --- LÓGICA DE ADMINISTRADOR (REPARADA) ---
    ...
    // --- CORRECCIÓN: Evento de edición ahora incluye el ROL ---
    socket.on('edit_user', ({ username, newFullName, newPassword, newRole }) => {
        if (hasAdminPermission() && username) {
            const userIndex = users.findIndex(u => u.user === username);
            if (userIndex > -1) {
                users[userIndex].fullName = newFullName;
                users[userIndex].pass = newPassword;
                users[userIndex].role = newRole; // Se actualiza el rol
                saveUsers();
                io.emit('users_update', users);
            }
        }
    ...
    // ... (El resto del código de guardias y pacientes no necesita cambios para esta corrección)
...
server.listen(DEPLOY_PORT, '0.0.0.0', () => {
    loadData();
    console.log(`✔️  Servidor SIG v3.4 escuchando en el puerto ${DEPLOY_PORT}`);
